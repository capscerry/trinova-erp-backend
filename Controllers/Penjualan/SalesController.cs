...
namespace trinova_erp_backend.Controllers.Penjualan
{
    ...
    public class SalesController : ControllerBase
    {
        ...
        public async Task<IActionResult> SendQuotationEmail(int quotationId, [FromBody] SendQuotationEmailRequest? request)
        {
            try
            {
                await _salesQuotationUsecase.SendQuotationEmailAsync(quotationId, request);
                return Ok(new { success = true, message = "Email penawaran (PDF) berhasil dikirim ke pelanggan." });
            }
