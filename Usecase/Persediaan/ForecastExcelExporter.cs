...
namespace trinova_erp_backend.Usecase.Persediaan
{
    public class ForecastExcelExporter
    {
        public byte[] Export(List<ForecastResult> forecasts)
        {
            ...
            var averageForecast  = forecasts.Any() ? forecasts.Average(x => x.ForecastNextMonth) : 0;
            var forecastMonth    = forecasts.FirstOrDefault()?.ForecastMonth ?? "-";

            var highestForecastProduct = forecasts.OrderByDescending(x => x.ForecastNextMonth).FirstOrDefault();
            ...
            // ── Forecast Data sheet content ───────────────────────────────────

            var row = 2;
            foreach (var forecast in forecasts)
            {
                var forecastMonthText  = forecast.ForecastMonth;
                if (DateTime.TryParse($"{forecast.ForecastMonth}-01", out var fDate))
                    forecastMonthText = fDate.ToString("MMMM yyyy");

                var trainingPeriodText = forecast.LastTrainingPeriod;
                if (DateTime.TryParse($"{forecast.LastTrainingPeriod}-01", out var tDate))
                    trainingPeriodText = tDate.ToString("MMMM yyyy");
